############################################
###       Garcia et al. (2005)           ###
# --- modification with noisy predictors ###
############################################
...
train_splits <- readRDS(file = "../99_Stored_Data/garcia_data_splits_train_noise.rds")
test_splits <- readRDS(file = "../99_Stored_Data/garcia_data_splits_test_noise.rds")
...
# mc_iter <- 1
  ...
  ### LASSO -------
  lasso_fit <- cv.glmnet(x=as.matrix(train_splits[[mc_iter]][,c(2:29)]), y = train_splits[[mc_iter]][,c(1)], alpha = 1, nfolds = 10)
  lasso_pred <-predict(lasso_fit, newx = as.matrix(test_splits[[mc_iter]][,c(2:29)]), s = "lambda.min")
...
saveRDS(mse, file = "../99_Stored_Data/garcia_benchmarks_noise.rds")

boost_objects <- list(gam1[m_stop_cvm], glm1[m_stop_aic])
saveRDS(boost_objects, file = "../99_Stored_Data/garcia_boost_objects_noise.rds")
